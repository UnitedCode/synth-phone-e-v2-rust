        ...
        for (left, right) in &buffer.as_slice()[..BLOCK_SIZE] {
            ...
            shared.in_buffer.lock(|in_buffer| {
                shared.in_buffer_pointer.lock(|in_buffer_pointer| {
                    in_buffer[*in_buffer_pointer as usize] = sample;
...
pub fn dma1_stream0_software_task(
    ...
    ctx.out_buffer_write_pointer
        .lock(|out_buffer_write_pointer| {
            *out_buffer_write_pointer =
                (*out_buffer_write_pointer + HOP_SIZE as u32) % BUFFER_SIZE as u32;
        });
}
...
pub fn process_dry(ctx: &mut crate::rtic_app::app::dma1_stream0_software_task::SharedResources) {
    ...
    for i in 0..FFT_SIZE {
        ctx.in_buffer_pointer_cached
            .lock(|in_buffer_pointer_cached| {
                let circular_buffer_index =
                    (*in_buffer_pointer_cached + i as u32 - FFT_SIZE as u32 + BUFFER_SIZE as u32)
                        % BUFFER_SIZE as u32;
                ctx.in_buffer.lock(|in_buffer| {
                    unwrapped_buffer[i] =
                        in_buffer[circular_buffer_index as usize] * analysis_window_buffer[i];
                });
                ctx.carrier_buffer.lock(|carrier_buffer| {
                    unwrapped_synth_buffer[i] =
                        carrier_buffer[circular_buffer_index as usize] * analysis_window_buffer[i];
                });
            });
    }
    ...
    for i in 0..FFT_SIZE {
        ctx.out_buffer_write_pointer
            .lock(|out_buffer_write_pointer| {
                let circular_buffer_index =
                    (*out_buffer_write_pointer + i as u32) % BUFFER_SIZE as u32;
                ...
                ctx.out_buffer.lock(|out_buffer| {
                    out_buffer[circular_buffer_index as usize] +=
                        result * analysis_window_buffer[i];
                });
            });
    }
}
    ...
    for i in 0..FFT_SIZE {
        ctx.out_buffer_write_pointer
            .lock(|out_buffer_write_pointer| {
                ...
                ctx.out_buffer.lock(|out_buffer| {
                    out_buffer[circular_buffer_index as usize] +=
                        res[i].re * analysis_window_buffer[i];
                });
            });
    }
}
